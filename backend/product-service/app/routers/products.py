...
async def create_parent_category(category: CategoryBase):
    ...
    if category.parent_id:
        raise HTTPException(status_code=400, detail="Parent categories cannot have a parent_id")
    ...
    if "subcategories" in category:
        ...
        for sub in category["subcategories"]:
            if isinstance(sub, dict) and "_id" in sub and re.match(r'^[0-9a-fA-F]{24}$', sub["_id"]):
                ...
                print(f"Skipping invalid subcategory entry: {sub}")
        category_data["subcategories"] = subcategories
    return category_data

@router.get("/categories/{specific_category_id}", response_model=ParentCategory | ChildCategory)
async def get_category(specific_category_id: str):
    db = get_database()
    category = await fetch_category_with_subcategories(specific_category_id, db)
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
    if category.get("parent_id") is None:
        return ParentCategory(**category, id=category["_id"])
    return ChildCategory(**category, id=category["_id"])

@router.get("/categories/parents-with-subcategories", response_model=List[ParentCategory])
        ...
        if "subcategories" in category:
            ...
            for sub in category["subcategories"]:
                if isinstance(sub, dict) and "_id" in sub and re.match(r'^[0-9a-fA-F]{24}$', sub["_id"]):
                    ...
                    sub_cat = await db.categories.find_one({"_id": ObjectId(sub["_id"])})
                    if sub_cat:
                        processed_sub = {
                            "_id": str(sub_cat["_id"]),
                            "name": sub_cat["name"],
                            "parent_id": str(sub_cat["parent_id"]) if sub_cat.get("parent_id") else None,
...
async def get_all_parent_categories():
    ...
    async for category in cursor:
        ...
        processed_category = {
            ...
            "parent_id": None,
            "image": category.get("image"),
            "subcategories": []  # Exclude subcategories
        }
        parent_categories.append(ParentCategory(**processed_category))
    return parent_categories
...
async def create_product(product: ProductBase):
    ...
    if category.get("parent_id"):
        ...
        if parent and not parent.get("parent_id"):
            raise HTTPException(status_code=400, detail="Products are not allowed for subcategories of parent categories.")
...
async def get_products_in_subcategory(subcategory_id: str, page: int = 1, limit: int = 10):
    ...
    if subcategory.get("parent_id"):
        parent = await db.categories.find_one({"_id": ObjectId(subcategory["parent_id"])})
        if parent and not parent.get("parent_id"):
...
async def update_product(product_id: str, updated_data: ProductBase):
    ...
    if "category_id" in updated_data.dict() and updated_data.category_id != product["category_id"]:
        ...
        if category.get("parent_id"):
            parent = await db.categories.find_one({"_id": ObjectId(category["parent_id"])})
            if parent and not parent.get("parent_id"):
                raise HTTPException(status_code=400, detail="Products cannot be assigned to subcategories of parent categories.")
        await db.categories.update_one({"_id": ObjectId(product["category_id"])}, {"$pull": {"products": oid}})
        await db.categories.update_one({"_id": category_oid}, {"$push": {"products": oid}})
...
async def search_products(query: str, page: int = 1, limit: int = 10):
    db = get_database()
    skip = (page - 1) * limit
    product_cursor = db.products.find({"name": {"$regex": query, "$options": "i"}}).skip(skip).limit(limit)
    products = []
    async for product in product_cursor:
        product["_id"] = str(product["_id"])
        product["category_id"] = str(product["category_id"])
...
async def filter_products(
    ...
    if sim_card: query["sim_card"] = sim_card
    if processor_type: query["processor_type"] = processor_type
...
async def autocomplete_products(query: str = Query(..., min_length=1, max_length=50)):
    ...
    if cached_data:
        return json.loads(cached_data)
    cursor = db.products.find({"name": {"$regex": f"^{query}", "$options": "i"}}).limit(5)
...
async def delete_category(category_id: str):
    ...
    if category.get("parent_id"):
        await db.categories.update_one({"_id": ObjectId(category["parent_id"])}, {"$pull": {"subcategories": oid}})
...
async def delete_product(product_id: str):
    ...
    await db.categories.update_one({"_id": ObjectId(category_id)}, {"$pull": {"products": oid}})
